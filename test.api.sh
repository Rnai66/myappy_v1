...
CREATE_RESPONSE=$(curl -s -X POST $BASE_URL/listings \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{"title":"First Item","description":"My first product","price":100}')
...
curl -s -X PUT $BASE_URL/listings/$LISTING_ID \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{"title":"Updated Item","description":"Updated description","price":200}'
echo -e "\n"
